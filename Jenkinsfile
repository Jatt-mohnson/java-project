...
node('linux') {   
	...
	stage('Build') {    
		...
		sh 'aws cloudformation create-stack --region us-west-1 --stack-name myteststack --template-url https://s3.amazonaws.com/seis665/jenkins-cf.json --parameters ParameterKey=JenkinsPassword,ParameterValue=test1 ParameterKey=JenkinsSlaveNum,ParameterValue=1'
	}   
	stage('Deploy') {    
